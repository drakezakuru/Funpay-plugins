...
NAME = "Steam Rental"
VERSION = "2.23.8"
DEBUG_BUILD = "mobileconf-skip-direct-with-proxy-v6-nocooldown"
DESCRIPTION = (
...
_ACTION_ICONS = {
    ...
    "acc_chpwd":       "🔑 СМЕНА PW",
    "acc_chpwd_fail":  "❌ PW FAIL ",
    "revoke":          "🚪 ОТЗЫВ   ",
    "revoke_fail":     "⚠ ОТЗЫВ FA",
    "bulk_chpwd":      "🔁 BULK PW ",
    "mobileconf_test": "📱 MCNF TEST",
    "cfg_change":      "⚙ НАСТРОЙКА",
    "review_bonus":    "⭐ БОНУС   ",
...
def _read_actions_log_full() -> str:
    """Полный actions.log + бэкапы ротации (кнопка «📜 Логи» в Инструментах)."""
    parts: list[str] = []
    try:
        for i in range(ACTIONS_LOG_BACKUPS, -1, -1):
            p = ACTIONS_LOG_FILE if i == 0 else f"{ACTIONS_LOG_FILE}.{i}"
            if not os.path.isfile(p):
                continue
            with open(p, "r", encoding="utf-8", errors="replace") as f:
                content = f.read().strip()
            if content:
                parts.append(f"----- actions.log.{i} -----")
                parts.append(content)
    except Exception:
        LOGGER.debug("steam_rental: чтение actions.log для выгрузки не удалось",
                     exc_info=True)
    return "\n".join(parts)


def _clear_actions_log_files() -> None:
    """Удаляет actions.log и бэкапы ротации (кнопка «🧹 Очистить»)."""
    try:
        ...
        LOGGER.debug("steam_rental: очистка actions.log не удалась",
                     exc_info=True)


def _log_cfg_change(key: str, value: Any, *, source: str = "menu",
                    user: Any = None) -> None:
    """Фиксирует в actions.log смену настройки через меню.

    Чувствительные значения (ключи/пароли/токены/прокси и т.п.)
    маскируются по имени ключа.
    """
    sv = str(value)
    low_key = key.lower()
    if any(h in low_key for h in
           ("password", "secret", "token", "key", "cookie", "pin", "cvv",
            "proxy", "url", "golden")):
        sv = "[REDACTED]"
    elif len(sv) > 120:
        sv = sv[:117] + "…"
    extra: dict[str, Any] = {"key": key, "value": sv, "source": source}
    if user is not None:
        extra["user"] = user
    _log_action("cfg_change", f"Настройка {key} изменена", **extra)
...
def _track_login_result(alias: str, success: bool) -> None:
    ...
    with _lock:
        ...
        upsert_account(acc)
    if success:
        _log_action("acc_login_ok", f"Логин {alias} OK", alias=alias)
    else:
        _log_action("acc_login_fail",
                    f"Логин {alias} не удался",
                    alias=alias, failures=fail_count)
    if triggered:
...
def _finish_rental(acc: dict[str, Any], *, reason: str = "manual_finish",
    ...
    try:
        ...
        if do_chpwd:
            ...
            try:
                ...
                _log_action("acc_chpwd", f"Пароль изменён: {alias}",
                            alias=alias, reason=reason)
            except Exception as exc:
                LOGGER.warning(
                    "steam_rental: _finish_rental: change_password failed "
                    "for %s: %s", alias, exc)
                _log_action("acc_chpwd_fail",
                            f"Смена пароля не удалась: {alias}",
                            alias=alias, reason=reason,
                            error=_short_proxy_error(exc)[:200])
    except Exception:
    ...
    if do_revoke:
        try:
            ...
            _log_action("revoke", f"Сессии отозваны: {alias}",
                        alias=alias, reason=reason)
        except Exception:
            LOGGER.warning(
                "steam_rental: _finish_rental: revoke failed for %s",
                alias, exc_info=True)
            _log_action("revoke_fail", f"Отзыв сессий не удался: {alias}",
                        alias=alias, reason=reason)
    with _lock:
        acc.pop("rental", None)
...
def _end_rental_impl(cardinal: "Cardinal | None", alias: str, reason: str,
    ...
    if sess is not None and do_chpwd:
        ...
        if result.get("changed"):
            ...
            _last_err = str((result.get("errors") or [""])[-1])
            _log_action("acc_chpwd_fail", f"Смена пароля не удалась: {alias}",
                        alias=alias, reason=reason, error=_last_err[:200])
...
def _test_mobile_conf_for_account(alias: str, chat_id: int, bot: Any) -> None:
    ...
    _log_action("mobileconf_test", f"Mobile conf test запущен: {alias}",
                alias=alias)
    ...
    ok_cnt = sum(1 for ln in results if ln.lstrip().startswith("✅"))
    bad_cnt = sum(1 for ln in results
                  if ln.lstrip().startswith(("🔴", "❌")))
    _log_action("mobileconf_test",
                f"Mobile conf test: {alias} — ✅ {ok_cnt}, проблемы: {bad_cnt}",
                alias=alias, ok=ok_cnt, bad=bad_cnt)
...
def _register_tg_commands(cardinal: "Cardinal") -> None:
    ...
    def _text_logs() -> str:
        full = _read_actions_log_full()
        if not full:
            return "<b>📜 Логи плагина</b>\n\nactions.log пока пуст."
        tail = full[-3500:]
        return (
            "<b>📜 Логи плагина</b>\n"
            f"Файл: <code>{_esc(ACTIONS_LOG_FILE)}</code>\n"
            f"Ротация: {ACTIONS_LOG_MAX_BYTES // (1024 * 1024)} МБ × "
            f"{ACTIONS_LOG_BACKUPS}\n\n"
            f"<pre>{_esc(tail)}</pre>\n\n"
            "Показан хвост лога. Полные логи (включая бэкапы ротации) — "
            "кнопкой «📤 Скачать файл»."
        )

    def _kb_logs() -> tbtypes.InlineKeyboardMarkup:
        kb = tbtypes.InlineKeyboardMarkup(row_width=2)
        kb.add(
            tbtypes.InlineKeyboardButton(
                "📤 Скачать файл", callback_data="sr:logs_file"),
            tbtypes.InlineKeyboardButton(
                "🧹 Очистить", callback_data="sr:logs_clear"),
        )
    ...
    def _save_proxy_entries(cfg: dict, entries: list[dict[str, Any]]) -> None:
        ...
        save_config(cfg)
        _log_action("cfg_change", "Steam mobile proxies обновлены",
                    count=len(entries),
                    proxies=", ".join(_mask_proxy(e.get("url", ""))
    ...
    def _bulk_chpwd_thread(chat_id, msg_id):
        ...
        fail = 0
        for acc in targets:
            blk_alias = acc.get("alias", "?")
            try:
                s = _steam_session_from_acc(acc)
                s.login()
                _track_login_result(blk_alias, True)
                new_pw = _gen_password()
                s.change_password(new_pw)
                with _lock:
                    a = find_account(blk_alias) or acc
                    a["password"] = new_pw
                    ...
                    upsert_account(a)
                ok += 1
                _log_action("bulk_chpwd", f"Массовая смена пароля: {blk_alias}",
                            alias=blk_alias)
            except Exception as exc:
                _track_login_result(blk_alias, False)
                fail += 1
                _log_action("acc_chpwd_fail",
                            f"Массовая смена пароля не удалась: {blk_alias}",
                            alias=blk_alias,
                            error=_short_proxy_error(exc)[:200])
            time.sleep(3)
        kb = tbtypes.InlineKeyboardMarkup()
    ...
    def on_cb(call):
        ...
        try:
            ...
            # сохраняя контекст (sid → rental → alias). Без выбора аккаунта.
                ...
                save_config(cfg)
                _log_cfg_change("expire_action", cfg["expire_action"],
                                source="expact", user=uid)
                ...
                cfg["steam_mobile_proxies_updated_at"] = _now()
                save_config(cfg)
                _log_action("cfg_change", "Steam mobile proxies очищены",
                            source="proxyclear", user=uid)
                ...
                if arg in cfg and isinstance(cfg[arg], bool):
                    cfg[arg] = not cfg[arg]
                    save_config(cfg)
                    _log_cfg_change(arg, cfg[arg], source="toggle", user=uid)
                ...
                _edit_menu(chat_id, msg_id, _text_logs(), _kb_logs())
                tg.bot.answer_callback_query(call.id)
                ...
                try:
                    ...
                    if not full:
                        tg.bot.answer_callback_query(
                            call.id, "actions.log пока пуст.")
                    else:
                        doc = io.BytesIO(full.encode("utf-8"))
                        doc.name = "steam_rental_actions.log"
                        tg.bot.send_document(
                            chat_id, doc,
                            caption="📜 Логи Steam Rental (полные, "
                                    "включая ротацию)")
                        tg.bot.answer_callback_query(call.id, "Лог отправлен.")
                except Exception as exc:
                    tg.bot.answer_callback_query(
                ...
                _clear_actions_log_files()
                tg.bot.answer_callback_query(call.id, "Логи очищены.")
                _edit_menu(chat_id, msg_id, _text_logs(), _kb_logs())
            elif action == "bulk":
    ...
    def _handle_pending_text(message):
        ...
        # ── Wizard: лот для продления (extension-лот) ─────────────────
            ...
            save_config(cfg)
            _log_cfg_change(setting_key, cfg[setting_key],
                            source="edit", user=uid)
            _pending_state.pop(uid, None)
...
def _srp_handle_callback(cardinal: "Cardinal", bot: Any, call: Any) -> None:
    ...
    try:
        ...
        if action == "menu":
            ...
            save_config(cfg)
            _log_cfg_change(param, cfg[param], source="rp_toggle")
